...
client.on("message", message => { //clear
              var args = message.content.substring(prefix.length).split("1 ");
              if (message.content.startsWith(prefix + "clear")) {
                  if(!message.channel.guild) return message.reply('**? اسف لكن هذا الامر للسيرفرات فقط **');         
     if(!message.member.hasPermission('Team Red الرجاء تجيب زوار الى سيرفر ')) return message.reply('**?  لا يوجد لديك صلاحية لمسح الشات**');
          var msg;
          msg = parseInt();
...
client.login(NjQ2MzgwNDg2ODQ3MTAzMDE3.XdWU6g.yH5-ROOIkZ6hNZMCo7lAKdHlxSQ);